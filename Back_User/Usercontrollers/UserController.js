const User = require('../UsertModels/UserModels');
...
async function addUser(req, res) {
  try {
    ...
    if (!userData.email || !userData.password) {
      return res.status(400).json({ error: 'El correo electrónico y la contraseña son campos obligatorios' });
    }
    ...
    res.json({ Mensaje: 'Usuario creado correctamente', createdUser: userToBeAdded });
  } catch (error) {
...
async function updateUser(req, res) {
  try {
    ...
    return res.json({ Mensaje: `Usuario actualizado: ${idOfUserToUpdate}`, updatedUser });
  } catch (error) {
...
async function deleteUser(req, res) {
  try {
    ...
    return res.json({ Mensaje: `Usuario borrado: ${idOfUserToBeDeleted}` });
  } catch (error) {
